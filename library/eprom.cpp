#include "eprom.h"

const char * const EPROM::_name[] = {"24AA01","24AA02","24AA04","24AA08","24AA16","24AA32",
                                        "24AA64","24AA128","24AA256","24AA512","24AA1024","24AA1025"};
                                        
EPROM::EPROM(PinName sda, PinName scl, uint8_t address, TypeEeprom type) : _i2c(sda, scl) 
{
  
  _errnum = EEPROM_NoError;
  _type = type;
  
  // Check address range
  _address = address;
  switch(type) {
                 case T24AA01 :
                 case T24AA02 :
                    if(address > 7) {
                      _errnum = EEPROM_BadAddress;
                    }
                    _address = _address << 1;
                    _page_write = 8;
                    _page_number = 1;
                    break;
                 case T24AA04 :
                    if(address > 7) {
                      _errnum = EEPROM_BadAddress;
                    }
                    _address = (_address & 0xFE) << 1;
                    _page_write = 16;
                    _page_number = 2;
                    break;
                 case T24AA08 :
                    if(address > 7) {
                      _errnum = EEPROM_BadAddress;
                    }
                    _address = (_address & 0xFC) << 1;
                    _page_write = 16;
                    _page_number = 4;
                    break;
                 case T24AA16 :
                    _address = 0;
                    _page_write = 16;
                    _page_number = 8;
                    break;
                  case T24AA32 :
                  case T24AA64 :
                    if(address > 7) {
                      _errnum = EEPROM_BadAddress;
                    }
                    _address = _address << 1;
                    _page_write = 32;
                    _page_number = 1;
                    break;
                  case T24AA128 :
                  case T24AA256 :
                    if(address > 3) {
                      _errnum = EEPROM_BadAddress;
                    }
                    _address = _address << 1;
                    _page_write = 64;
                    _page_number = 1;
                    break;
                 case T24AA512 :
                    if(address > 3) {
                      _errnum = EEPROM_BadAddress;
                    }
                    _address = _address << 1;
                    _page_write = 128;
                    _page_number = 1;
                    break;
                 case T24AA1024 :
                    if(address > 7) {
                      _errnum = EEPROM_BadAddress;
                    }
                    _address = (_address & 0xFE) << 1;
                    _page_write = 128;
                    _page_number = 2;
                    break;
                 case T24AA1025 :
                    if(address > 3) {
                      _errnum = EEPROM_BadAddress;
                    }
                    _address = _address << 1;
                    _page_write = 128;
                    _page_number = 2;
                    break;
  }
  
  // Size in bytes
  _size = _type;
  if(_type == T24AA1025)
    _size = T24AA1024;
  
  // Set I2C frequency
  _i2c.frequency(EEPROM_Freq);

}

/**
 * int write(uint32_t address, int8_t data)
 *
 * Write byte
 * @param address start address (uint32_t)
 * @param data byte to write (int8_t)
 * @return none
*/
int EPROM::write(uint32_t address, int8_t data)
{
  uint8_t page;
  uint8_t addr;
  uint8_t cmd[3];
  int len;
  int ack;
    
  // Check error
  if(_errnum) 
    return _errnum;
    
  // Check address
  if(!checkAddress(address)) {
    _errnum = EEPROM_OutOfRange;
    return _errnum;
  }
  
  // Compute page number
  page = 0;
  if(_type < T24AA32)
    page = (uint8_t) (address / 256); 
  
  // Device address
  addr = EEPROM_Addr | _address | (page << 1);
 
  if(_type < T24AA32) {
    len = 2;
    
    // Word address 
    cmd[0] = (uint8_t) (address - page * 256);
  
    // Data
    cmd[1] = (uint8_t) data;
  }
  else {
    len = 3;
    
    // First word address (MSB)
    cmd[0] = (uint8_t) (address >> 8);
    
    // Second word address (LSB)
    cmd[1] = (uint8_t) address;
    
    // Data
    cmd[2] = (uint8_t) data;
  }
    
  ack = _i2c.write((int)addr,(char *)cmd,len);
  if(ack != 0) {
    _errnum = EEPROM_I2cError;
    return _errnum;
  }
  
  // Wait end of write
  ready();
  return EEPROM_NoError;
}

/**
 * int write(uint32_t address, int8_t data[], uint32_t length)
 *
 * Write array of bytes (use the page mode)
 * @param address start address (uint32_t)
 * @param data bytes array to write (int8_t[])
 * @param size number of bytes to write (uint32_t)
 * @return none
*/
int EPROM::write(uint32_t address, int8_t data[], uint32_t length)
{
  uint8_t page;
  uint8_t addr = 0;
  uint8_t blocs,remain;
  uint8_t fpart,lpart;
  uint8_t i,j,ind;
  uint8_t cmd[129];
  int ack;
    
  // Check error
  if(_errnum) 
    return _errnum;
    
  // Check address
  if(!checkAddress(address)) {
    _errnum = EEPROM_OutOfRange;
    return _errnum;
  }
  
  // Check length
  if(!checkAddress(address + length - 1)) {
    _errnum = EEPROM_OutOfRange;
    return _errnum;
  }
  
  // Compute blocs numbers
  blocs = length / _page_write;
  
  // Compute remaining bytes
  remain = length - blocs * _page_write;
    
  for(i = 0;i < blocs;i++) {
     // Compute page number
     page = 0;
     if(_type < T24AA32)
       page = (uint8_t) (address / 256); 
  
     // Device address
     addr = EEPROM_Addr | _address | (page << 1);
  
     if(_type < T24AA32) {
       // Word address
       cmd[0] = (uint8_t) (address - page * 256);
     
       if((uint8_t) ((address + _page_write) / 256) == page) { // Data fit in the same page
         // Add data 
         for(j = 0;j < _page_write;j++)
            cmd[j + 1] = (uint8_t) data[i * _page_write + j];
    
         // Write data
         ack = _i2c.write((int)addr,(char *)cmd,_page_write + 1);
         if(ack != 0) {
           _errnum = EEPROM_I2cError;
           return _errnum;
         }
  
         // Wait end of write
         ready();
       
         // Increment address
         address += _page_write;
       }
       else { // Data on 2 pages. We must split the write
         // Number of bytes in current page
         fpart = (page + 1) * 256 - address;
       
         // Add data for current page
         for(j = 0;j < fpart;j++)
            cmd[j + 1] = (uint8_t) data[i * _page_write + j];
    
         // Write data for current page
         ack = _i2c.write((int)addr,(char *)cmd,fpart + 1);
         if(ack != 0) {
           _errnum = EEPROM_I2cError;
           return _errnum;
         }
  
         // Wait end of write
         ready();
       
         // Increment address
         address += fpart;
       
         if(page < _page_number - 1) {
           // Increment page
           page++;
       
           // Device address
           addr = EEPROM_Addr | _address | (page << 1);
       
           // Word address
           cmd[0] = (uint8_t) (address - page * 256);
       
           // Data index
           ind = i * _page_write + fpart;
       
           // Number of bytes in next page
           lpart = _page_write - fpart;
       
           // Add data for next page
           for(j = 0;j < lpart;j++)
              cmd[j + 1] = (uint8_t) data[ind + j];
    
           // Write data for next page
           ack = _i2c.write((int)addr,(char *)cmd,lpart + 1);
           if(ack != 0) {
             _errnum = EEPROM_I2cError;
             return _errnum;
           }
  
           // Wait end of write
           ready();
       
           // Increment address
           address += lpart;
         }
       }
     }
     else {
       // First word address (MSB)
       cmd[0] = (uint8_t) (address >> 8);
       
       // Second word address (LSB)
       cmd[1] = (uint8_t) address;
       
       // Add data 
       for(j = 0;j < _page_write;j++)
          cmd[j + 2] = (uint8_t) data[i * _page_write + j];
    
       // Write data
       ack = _i2c.write((int)addr,(char *)cmd,_page_write + 2);
       if(ack != 0) {
         _errnum = EEPROM_I2cError;
         return _errnum;
       }
  
       // Wait end of write
       ready();
       
       // Increment address
       address += _page_write;
     }
  }
  
  if(remain) {
    // Compute page number
    page = 0;
    if(_type < T24AA32)
      page = (uint8_t) (address / 256); 
  
    // Device address
    addr = EEPROM_Addr | _address | (page << 1);
  
    if(_type < T24AA32) {
      // Word address
      cmd[0] = (uint8_t) (address - page * 256);
  
      if((uint8_t) ((address + remain) / 256) == page) { // Data fit in the same page
        // Add data for the current page
        for(j = 0;j < remain;j++)
           cmd[j + 1] = (uint8_t) data[blocs * _page_write + j];
    
        // Write data for the current page
        ack = _i2c.write((int)addr,(char *)cmd,remain + 1);
        if(ack != 0) {
          _errnum = EEPROM_I2cError;
          return _errnum;
        }
  
        // Wait end of write
        ready();
      }
      else { // Data on 2 pages. We must split the write
        // Number of bytes in current page
        fpart = (page + 1) * 256 - address;
    
        // Add data for current page
        for(j = 0;j < fpart;j++)
           cmd[j + 1] = (uint8_t) data[blocs * _page_write + j];
    
        // Write data for current page
        ack = _i2c.write((int)addr,(char *)cmd,fpart + 1);
        if(ack != 0) {
          _errnum = EEPROM_I2cError;
          return _errnum;
        }
  
        // Wait end of write
        ready();
    
        // Increment address
        address += fpart;
       
        if(page < _page_number - 1) {
          // Increment page
          page++;
    
          // Device address
          addr = EEPROM_Addr | _address | (page << 1);
    
          // Word address
          cmd[0] = (uint8_t) (address - page * 256);
    
          // Data index
          ind = blocs * _page_write + fpart;
    
          // Number of bytes in next page
          lpart = remain - fpart;
    
          // Add data for next page
          for(j = 0;j < lpart;j++)
             cmd[j + 1] = (uint8_t) data[ind + j];
    
          // Write data for next page
          ack = _i2c.write((int)addr,(char *)cmd,lpart + 1);
          if(ack != 0) {
            _errnum = EEPROM_I2cError;
            return _errnum;
          }
        
          // Wait end of write
          ready();
        }
      }
    }
    else {
      // Fist word address (MSB)
      cmd[0] = (uint8_t) (address >> 8);
    
      // Second word address (LSB)
      cmd[1] = (uint8_t) address;

      // Add data for the current page
      for(j = 0;j < remain;j++)
         cmd[j + 2] = (uint8_t) data[blocs * _page_write + j];
        
      // Write data for the current page
      ack = _i2c.write((int)addr,(char *)cmd,remain + 2);
      if(ack != 0) {
        _errnum = EEPROM_I2cError;
        return _errnum;
      }
  
      // Wait end of write
      ready();
        }
  }
  return EEPROM_NoError;
}

/**
 * int write(uint32_t address, int16_t data)
 *
 * Write short
 * @param address start address (uint32_t)
 * @param data short to write (int16_t)
 * @return none
*/
int EPROM::write(uint32_t address, int16_t data)
{
  int8_t cmd[2];
    
  // Check error
  if(_errnum) 
    return _errnum;
    
  // Check address
  if(!checkAddress(address + 1)) {
    _errnum = EEPROM_OutOfRange;
    return _errnum;
  }
  
  memcpy(cmd,&data,2);
  
  write(address,cmd,2);
  return EEPROM_NoError; 
}

/**
 * int write(uint32_t address, int32_t data)
 *
 * Write long
 * @param address start address (uint32_t)
 * @param data long to write (int32_t)
 * @return none
*/
int EPROM::write(uint32_t address, int32_t data)
{
  int8_t cmd[4];
    
  // Check error
  if(_errnum) 
    return _errnum;
    
  // Check address
  if(!checkAddress(address + 3)) {
    _errnum = EEPROM_OutOfRange;
    return _errnum;
  }
  
  memcpy(cmd,&data,4);
  
  write(address,cmd,4);
  return EEPROM_NoError;
}

/**
 * int write(uint32_t address, float data)
 *
 * Write float
 * @param address start address (uint32_t)
 * @param data float to write (float)
  * @return none
*/
int EPROM::write(uint32_t address, float data)
{
  int8_t cmd[4];
    
  // Check error
  if(_errnum) 
    return _errnum;
    
  // Check address
  if(!checkAddress(address + 3)) {
    _errnum = EEPROM_OutOfRange;
    return _errnum;
  }
  
  memcpy(cmd,&data,4);
  
  write(address,cmd,4);
  return EEPROM_NoError;
}

/**
 * int write(uint32_t address, void *data, uint32_t size)
 *
 * Write anything (use the page write mode)
 * @param address start address (uint32_t)
 * @param data data to write (void *)
 * @param size number of bytes to write (uint32_t)
 * @return none
*/
int EPROM::write(uint32_t address, void *data, uint32_t size)
{
  int8_t *cmd = NULL;
    
  // Check error
  if(_errnum) 
    return _errnum;
    
  // Check address
  if(!checkAddress(address + size - 1)) {
    _errnum = EEPROM_OutOfRange;
    return _errnum;
  }
  
  cmd = (int8_t *)malloc(size);
  if(cmd == NULL) {
    _errnum = EEPROM_MallocError;
    return _errnum;
  }
    
  memcpy(cmd,(uint8_t *)data,size);
  
  write(address,cmd,size);
    
  free(cmd);
  return EEPROM_NoError;
}

/**
 * int read(uint32_t address, int8_t& data)
 *
 * Random read byte
 * @param address start address (uint32_t)
 * @param data byte to read (int8_t&)
 * @return none
*/
int EPROM::read(uint32_t address, int8_t& data)
{
  uint8_t page;
  uint8_t addr;
  uint8_t cmd[2];
  uint8_t len;
  int ack;
    
  // Check error
  if(_errnum) 
    return _errnum;
    
  // Check address
  if(!checkAddress(address)) {
    _errnum = EEPROM_OutOfRange;
    return _errnum;
  }
    
  // Compute page number
  page = 0;
  if(_type < T24AA32)
    page = (uint8_t) (address / 256); 
  
  // Device address
  addr = EEPROM_Addr | _address | (page << 1);
  
  if(_type < T24AA32) {
    len = 1;
    
    // Word address
    cmd[0] = (uint8_t) (address - page * 256);
  }
  else {
    len = 2;
    
    // First word address (MSB)
    cmd[0] = (uint8_t) (address >> 8);
    
    // Second word address (LSB)
    cmd[1] = (uint8_t)address;
  }
  
  // Write command
  ack = _i2c.write((int)addr,(char *)cmd,len,true);
  if(ack != 0) {
    _errnum = EEPROM_I2cError;
    return _errnum;
  }
  
  // Read data
  ack = _i2c.read((int)addr,(char *)&data,sizeof(data));
  if(ack != 0) {
    _errnum = EEPROM_I2cError;
    return _errnum;
  }
  return EEPROM_NoError;
}

/**
 * int read(uint32_t address, int8_t *data, uint32_t size)
 *
 * Sequential read byte
 * @param address start address (uint32_t)
 * @param data bytes array to read (int8_t[]&)
 * @param size number of bytes to read (uint32_t)
 * @return none
*/
int EPROM::read(uint32_t address, int8_t *data, uint32_t size)
{
  uint8_t page;
  uint8_t addr;
  uint8_t cmd[2];
  uint8_t len;
  int ack;
    
  // Check error
  if(_errnum) 
    return _errnum;
    
  // Check address
  if(!checkAddress(address)) {
    _errnum = EEPROM_OutOfRange;
    return _errnum;
  }
  
  // Check size
  if(!checkAddress(address + size - 1)) {
    _errnum = EEPROM_OutOfRange; 
    return _errnum;
  }
  
  // Compute page number
  page = 0;
  if(_type < T24AA32)
    page = (uint8_t) (address / 256); 
  
  // Device address
  addr = EEPROM_Addr | _address | (page << 1);
  
  if(_type < T24AA32) {
    len = 1;
    
    // Word address
    cmd[0] = (uint8_t) (address - page * 256);
  }
  else {
    len = 2;
    
    // First word address (MSB)
    cmd[0] = (uint8_t) (address >> 8);
    
    // Second word address (LSB)
    cmd[1] = (uint8_t) address;
  }
  
  // Write command 
  ack = _i2c.write((int)addr,(char *)cmd,len,true);
  if(ack != 0) {
    _errnum = EEPROM_I2cError;
    return _errnum;
  }
  
  // Sequential read
  ack = _i2c.read((int)addr,(char *)data,size);
  if(ack != 0) {
    _errnum = EEPROM_I2cError;
    return _errnum;
  }
  return EEPROM_NoError;
}

/**
 * int read(int8_t& data)
 *
 * Current address read byte
 * @param data byte to read (int8_t&)
 * @return none
 */
int EPROM::read(int8_t& data)
{
  uint8_t addr;
  int ack;
    
  // Check error
  if(_errnum) 
    return _errnum;
  
  // Device address
  addr = EEPROM_Addr | _address;

  // Read data
  ack = _i2c.read((int)addr,(char *)&data,sizeof(data));
  if(ack != 0) {
    _errnum = EEPROM_I2cError;
    return _errnum;
  }
  return EEPROM_NoError;
}

/**
 * int read(uint32_t address, int16_t& data)
 *
 * Random read short
 * @param address start address (uint32_t)
 * @param data short to read (int16_t&)
 * @return none
*/
int EPROM::read(uint32_t address, int16_t& data)
{
  int8_t cmd[2];
        
  // Check error
  if(_errnum) 
    return _errnum;
    
  // Check address
  if(!checkAddress(address + 1)) {
    _errnum = EEPROM_OutOfRange;
    return _errnum;
  }
 
  read(address,cmd,2);
  
  memcpy(&data,cmd,2);
  return EEPROM_NoError;
}

/**
 * int read(uint32_t address, int32_t& data)
 *
 * Random read long
 * @param address start address (uint32_t)
 * @param data long to read (int32_t&)
 * @return none
*/
int EPROM::read(uint32_t address, int32_t& data)
{
  int8_t cmd[4];
    
  // Check error
  if(_errnum) 
    return _errnum;
    
  // Check address
  if(!checkAddress(address + 3)) {
    _errnum = EEPROM_OutOfRange;
    return _errnum;
  }
 
  read(address,cmd,4);
  
  memcpy(&data,cmd,4);
  return 0;
}

/**
 * int read(uint32_t address, void *data, uint32_t size)
 *
 * Random read anything
 * @param address start address (uint32_t)
 * @param data data to read (void *)
 * @param size number of bytes to read (uint32_t)
 * @return none
*/
int EPROM::read(uint32_t address, void *data, uint32_t size)
{
  int8_t *cmd = NULL;
    
  // Check error
  if(_errnum) 
    return _errnum;
    
  // Check address
  if(!checkAddress(address + size - 1)) {
    _errnum = EEPROM_OutOfRange;
    return _errnum;
  }
  
  cmd = (int8_t *)malloc(size);

  if(cmd == NULL) {
    _errnum = EEPROM_MallocError;
    return _errnum;
  }
  
  read(address,(int8_t *)cmd,size);
  
  memcpy(data,cmd,size);
  
  free(cmd);
  return EEPROM_NoError;
}

/**
 * void clear(void)
 *
 * Clear eeprom (write with 0)
 * @param none
 * @return none
 */
void EPROM::clear(void)
{
  int32_t data;
  uint32_t i;
  
  data = 0;
  
  for(i = 0; i < _size / 4;i++) {
     write((uint32_t)(i * 4),data);  
  }
}

/**
 * void ready(void)
 *
 * Wait eeprom ready
 * @param none
 * @return none
*/
void EPROM::ready(void)
{
  int ack;
  uint8_t addr;
  uint8_t cmd[2];
  
  // Check error
  if(_errnum) 
    return;
  
  // Device address
  addr = EEPROM_Addr | _address;
  
  cmd[0] = 0;
  
  // Wait end of write
  do {
       ack = _i2c.write((int)addr,(char *)cmd,0);
           //wait(0.5);
  } while(ack != 0);

}

/**
 * uint32_t getSize(void)
 *
 * Get eeprom size in bytes
 * @param  none
 * @return size in bytes (uint32_t)
*/
uint32_t EPROM::getSize(void)
{
  return(_size);
}

/**
 * const char* getName(void)
 *
 * Get eeprom name
 * @param none
 * @return name (const char*)
 */
const char* EPROM::getName(void)
{
  uint8_t i = 0;
  
  switch(_type) {
                  case T24AA01 :
                                         i = 0;
                     break;
                  case T24AA02 :
                                         i = 1;
                     break;
                  case T24AA04 :
                                         i = 2;
                     break;
                  case T24AA08 :
                                         i = 3;
                     break;
                  case T24AA16 :
                                         i = 4;
                     break;
                  case T24AA32 :
                                         i = 5; 
                     break;
                  case T24AA64 :
                                         i = 6;
                     break;
                  case T24AA128 :
                                         i = 7;
                     break;
                  case T24AA256 :
                                         i = 8;
                     break;
                  case T24AA512 :
                                         i = 9;
                     break;
                  case T24AA1024 :
                                         i = 10;
                     break;
                  case T24AA1025 :
                                         i = 11;
                     break;
  }
  
  return(_name[i]);
}

/**
 * uint8_t getError(void)
 *
 * Get the current error number (EEPROM_NoError if no error)
 * @param none
 * @return none
*/
uint8_t EPROM::getError(void)
{
  return(_errnum);
}

/**
 * int checkAddress(uint32_t address)
 *
 * Check if address is in the eeprom range address
 * @param address address to check (uint32_t)
 * @return true if in eeprom range, overwise false (bool)
*/
bool EPROM::checkAddress(uint32_t address)
{
  bool ret = true;
  
  switch(_type) {
                  case T24AA01 :
                     if(address >= T24AA01)
                       ret = false;
                     break;
                  case T24AA02 :
                     if(address >= T24AA02)
                       ret = false;
                     break;
                  case T24AA04 :
                     if(address >= T24AA04)
                       ret = false;
                     break;
                  case T24AA08 :
                     if(address >= T24AA08)
                       ret = false;
                     break;
                  case T24AA16 :
                     if(address >= T24AA16)
                       ret = false;
                     break;
                  case T24AA32 :
                     if(address >= T24AA32)
                       ret = false;
                     break;
                  case T24AA64 :
                     if(address >= T24AA64)
                       ret = false;
                     break;
                  case T24AA128 :
                     if(address >= T24AA128)
                       ret = false;
                     break;
                  case T24AA256 :
                     if(address >= T24AA256)
                       ret = false;
                     break;
                  case T24AA512 :
                     if(address >= T24AA512)
                       ret = false;
                     break;
                  case T24AA1024 :
                     if(address >= T24AA1024)
                       ret = false;
                     break;
                  case T24AA1025 :
                     if(address >= T24AA1025 - 1)
                       ret = false;
                     break;
  }
  
  return(ret);
}  